...
class dbObject
{
	...
	private function __init() 
	{
		if($this->databaseInfo->ID != false) {
			...
			$input = dbConnection::getInstance()->fetchRow("select {$fieldnames} from {$this->databaseInfo->table} where {$this->databaseInfo->primary} = {$this->databaseInfo->ID}", 'mysql_fetch_assoc');
			$this->databaseValues = PostFilter::mysql2nl($input);
		}
	}
	 ...
	 * @returns int new primary key value
	 */
	 ...
	 * Analyzation happens by comparing primary keys. 
	 */
